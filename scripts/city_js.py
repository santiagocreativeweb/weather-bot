...
(function(){
  ...
  function statCards(C){var s=C.stats,c=[];
    ...
    var pk=C.picks[0];
    c.push(card('🔒 pick de hoy',pk?('<span style="color:var(--pick)">'+esc(pk.top[0]||'—')+'</span>'):'—',pk?(pk.kind==='24h'?'fijado 04:30 local':(pk.kind==='48h'?'fijado 48h':'preliminar')):'se fija 04:30 local'));
    if(s.pos)c.push(card('🏆 estabilidad','#'+s.pos,'de '+s.total+' ciudades'));
    return '<div class="sgrid">'+c.join('')+'</div>';
  }
  function picksBox(C){
    if(!C.picks.length)return '';
    var rows=C.picks.map(function(p){var kt=p.kind==='24h'?'🔒 24h fijado':(p.kind==='48h'?'⏳ 48h fijado':'◷ preliminar');
      return '<div class="pickrow"><span style="color:var(--mut)">'+esc(p.date)+'</span> · '+kt+' · μ '+p.mu+C.deg+'<br>&nbsp;&nbsp;'+pickHtml(p.top)+'</div>';}).join('');
    return '<div class="panelbox"><h4>🔒 Picks 24h / 48h — 🎯 exacto · 🥈 top-2 · 🥉 top-3</h4>'+rows+
      '<p class="subt" style="margin:6px 0 0">24h = fijado 04:30 local (lo que se opera). 48h = fijado un día antes (lo mide el tab 48hs de Estadísticas).</p></div>';
  }
  function mktBox(C){
  ...
  function histBox(C){
    ...
    var rows=C.history.map(function(r){var ic=IC[r.niv]||['⏳',''];return '<tr><td>'+esc(r.date)+'</td><td>'+esc(r.pick)+'</td><td>'+esc(r.win)+'</td><td><span class="gv '+ic[1]+'">'+ic[0]+' '+(r.niv||'pend.')+'</span></td></tr>';}).join('');
    return '<div class="panelbox"><h4>🗓 Historial</h4><table class="ct"><thead><tr><th>fecha</th><th>pick 🔒</th><th>ganó</th><th>resultado</th></tr></thead><tbody>'+rows+'</tbody></table></div>';
  }
  function tlBox(C){
...
(function(){
  ...
  function draw(){
    ...
    document.getElementById('cigrid').innerHTML=DATA.index.map(function(it){
      ...
      var picks=(it.picks||[]).map(function(p){var kt=p.kind==='24h'?'🔒24h':(p.kind==='48h'?'⏳48h':'◷');return '<div class="ci-pk"><span class="d">'+esc(p.date)+' '+kt+'</span> '+pickHtml(p.top)+'</div>';}).join('');
